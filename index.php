...
switch ($action) {
    ...
    case 'aicheck':
        ...
        $sys = "あなたは学習支援の先生です。学生のコードの問題点を『指摘』しますが、"
             ...
             . "severity は、実行を妨げる致命的な問題(文法エラー・未定義など)は \"error\"、動くが良くない/バグの可能性は \"warn\"、軽い気づきは \"info\"。"
    ...
    case 'sqlcheck':
        ...
        $sys = "あなたはPHPのSQLを可視化する補助ツールです。PHPコード内で、変数展開や文字列連結で組み立てているSQLクエリを見つけ、"
             . "『実際にデータベースへ送られるSQL文字列がどうなるか(展開結果)』を示すのが主目的です。脆弱性の説教はせず、展開を見せて学生自身に気づかせます。"
             . "各SQLについて hint(Markdown)には、まず (1)ふつうの入力例での展開結果、続けて (2)クォートなどを含むきわどい入力例での展開結果 を、それぞれ ``` のコードブロックで示してください。"
             . "展開結果を見れば壊れる/注入できることが分かるようにし、文章での補足は多くても1行まで。修正後の完成コードは書かない。"
             . "プレースホルダ(bind)で安全に組んだクエリは展開する必要が無いので issues に含めない。SQLが無ければ空配列。"
             . "severity は、きわどい入力で壊れる/注入できるものは \"error\"、それ以外の展開は \"info\"。"
             . "厳密なJSONだけを返す: {\"issues\":[{\"line\":整数, \"hint\":\"Markdown。展開結果のコードブロックが中心\", \"severity\":\"error|warn|info\"}]}";
        $usr = "ファイル名: {$filename}\n各行は「行番号<TAB>コード」の形式です。\n----\n{$numbered}";
