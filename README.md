...
`fixed_base => '/'` にすると、**サーバの任意のファイルを編集**できます(UI に赤い警告バナーが出ます)。
サーバの仕組みを学ぶ・実験する用途向けで、**本番では使わないこと**。設計上まったく安全ではないので、
**このアプリに誰がアクセスできるか**を厳しく絞ってください(学内IP限定・少数アカウントのみ 等)。
FPM プールは `open_basedir` を外し、プールユーザに対象ファイルの読み書き権限が必要です。
